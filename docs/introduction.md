...
- Addresses ambiguous base handling:
    ...
    - The ambiguous-base handling enables canonical 2-bit encoding of minimizer sequences, replacing FastANI’s Murmur3 minimizer hash digest as the lookup key. This improves determinism by eliminating hash-digest collisions and improves performance by avoiding repeated string-based Murmur3 hashing in the lookup representation.
...
- Scalability is a hard-requirement of this reimplementation. `fastANI` often looses favor to tools like `skani`, as large queries can explode memory and estimated runtime. The goal of this reimplementation is to scale the more accurate and sensitive `fastANI` algorithm to make it more practical in high-throughput environments
    - Leveraging SIMD processing significantly speeds up minimizer computations and other algorithm operations, making better use of modern compute resources. In addition, `fasterANI` replaces FastANI’s Murmur3-derived minimizer digest with canonical 2-bit-encoded minimizer keys: the hash is still used for minimizer selection, but the stored lookup key is the actual canonical k-mer encoding. This reduces hashing/key-materialization overhead, improves cache density by keeping keys as compact `u32`s, and removes digest-collision false positives.
    ...
    - Several options have been implemented to help scale to variable memory, processor and storage settings:
        ...
        - `--max-shard-size`: Controls persisted shard size using readable byte units
